...
func TestOnesCountAllNaive(t *testing.T) {
	leadingzeros.NaiveTest(t, table.LeadingZerosConcept)
	leadingzeros.NaiveTest8(t, table.LeadingZeros8)
	leadingzeros.NaiveTest16(t, table.LeadingZeros16)
	leadingzeros.NaiveTest32(t, table.LeadingZeros32)
	leadingzeros.NaiveTest64(t, table.LeadingZeros64)
}

func TestLeadingZerosConcept(t *testing.T) {
	cases := []struct {
		x        uint
		expected int
	}{
	...
	for _, c := range cases {
		got := table.LeadingZerosConcept(c.x)
		if got != c.expected {
			t.Errorf("table.LeadingZeros(0x%x) = %d, expected %d", c.x, got, c.expected)
		}
	}
}
...
func TestLeadingZeros8(t *testing.T) {
	...
	for _, c := range cases {
		got := table.LeadingZeros8(c.x)
		if got != c.expected {
			t.Errorf("table.LeadingZeros8(0x%02x) = %d, expected %d", c.x, got, c.expected)
		}
	}
}
...
func TestLeadingZeros16(t *testing.T) {
	...
	for _, c := range cases {
		got := table.LeadingZeros16(c.x)
		if got != c.expected {
			t.Errorf("table.LeadingZeros16(0x%04x) = %d, expected %d", c.x, got, c.expected)
		}
	}
}
...
func TestLeadingZeros32(t *testing.T) {
	...
	for _, c := range cases {
		got := table.LeadingZeros32(c.x)
		if got != c.expected {
			t.Errorf("table.LeadingZeros32(0x%08x) = %d, expected %d", c.x, got, c.expected)
		}
	}
}
...
func TestLeadingZeros64(t *testing.T) {
	...
	for _, c := range cases {
		got := table.LeadingZeros64(c.x)
		if got != c.expected {
			t.Errorf("table.LeadingZeros64(0x%016x) = %d, expected %d", c.x, got, c.expected)
		}
	}
}
